...
#     med_name = ["New Prince Medico", "Mahalaxmi Medico", "Dev Medical", "Bharat Healthcare & Lifestyle",
#                 "Shravani Medical", "Life Point Pharmacy", "Atlifeline Pharmacy"]
#     med_addr = ["Mahalunge Road, Baner","Swiss County Road, Thergaon","Bhumkar Chowk, Pune","Hinjewadi Phase 1, Pune","Wakad, Pune "]
#
...
#         report.medication = m_list
#
#     return render(request, 'patient/medications.html', {'Reports': all_reports, 's': s})
@login_required
def medications(request):
    patient = PatientProfile.objects.filter(patient=request.user).first()
    all_reports = Records.objects.filter(patient_id=patient.id).order_by('id').reverse()
    med_name = ["New Prince Medico", "Mahalaxmi Medico", "Dev Medical", "Bharat Healthcare & Lifestyle",
                "Shravani Medical", "Life Point Pharmacy", "Atlifeline Pharmacy"]
    med_addr = ["Ganesh plaza Shivaji chowk hinjewadi","Blue Ridge","Grampanchyat Building Chhatrapati Shivaji Maharaj Chowk","Hinjewadi Phase 1","Wakad","Near Sayaji Hotel" ,"Shivaji Chowk"]

    med_price = [round(random.uniform(10.00, 50.00), 2) for _ in range(5)]
    med_qyt = [random.randint(25, 100) for _ in range(5)]
    # Create a dictionary to map medication names to addresses
    med_data = {name: addr for name, addr in zip(med_name, med_addr)}
    ...
    return render(request, 'patient/medications.html', {'Reports': all_reports, 'med_qyt':med_qyt,'med_price' : med_price ,'med_name': med_name , 'med_addr' : med_addr})
